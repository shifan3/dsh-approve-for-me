...
// Static host plugin: install by pointing a loader row at this file.
// While enabled (config.enabled !== false), every approval request that
// reaches the answerer chain is judged by an LLM safety reviewer;
// destructive high-risk commands fall through to the interactive (user)
// answerer, and an unusable model also falls back to asking the user.
...
async function judgeWithLlm(req, llm, options) {
  const action = actionText(req);
  const summary = buildSummary(req.agent.session.events, options.summaryMaxChars);
  const system = [
...
export function apply(ctx, config) {
  ...
  const options = {
    provider: (config && config.provider) || "deepseek-official",
    model: (config && config.model) || "deepseek-v4-flash",
    maxTokens: (config && config.maxTokens) || 512,
    summaryMaxChars: (config && config.summaryMaxChars) || 12000,
    timeoutMs: (config && config.timeoutMs) || 60000,
    reasoningEffort: (config && config.reasoningEffort) || "off"
  };
  ctx.on("approval/request", async (req, next) => {
    try {
      ...
      const llm = ctx.llm;
      if (!llm) return next();
      const outcome = await judgeWithLlm(req, llm, options);
